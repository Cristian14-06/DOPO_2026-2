...
public class Triangle{
    ...
    public void equilatero(){
         ...
         width = (int)lado;
         height = (int)((Math.sqrt(3)/2)*lado);
         draw();
    }
    
    public void walk(int times){
        int avanceX = (int)(width/2);
        int avanceY = height;
        for(int i = 0; i < Math.abs(times); i++){
                if(times<0){
                    if(xPosition - avanceX > 0 && yPosition + avanceY > 0){
                        erase();
                        xPosition-=avanceX;
                        yPosition+=avanceY;
                        draw();
                ...
                if(xPosition + avanceX < 300 && yPosition + avanceY > 0){
                    erase();
                    xPosition+=avanceX;
                    yPosition+=avanceY;
                    draw();
    ...
    public int medidaLadoinclinado(){
        int lado = (int)(Math.sqrt(Math.pow(width/2,2)+Math.pow(height,2)));
        return lado;
}
}
